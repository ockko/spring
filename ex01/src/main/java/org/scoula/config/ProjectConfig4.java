...
public class ProjectConfig4 {
    ...
    // 2. 생성자()메서드 이용: 먼저 Parrot을 만들어두고, Person을 만들 때 미리 만들어놓은 parrot 객체를 주입하연 Person을 얻는다. 현재 제일 많이 사용함. 권장함.
    @Bean
    ...
    public Person person(Parrot parrot) {
        Person p = new Person(); // 객체생성 후, 주입하는 경우 set 메서드 이용해서 주입 가능!
        // Person p = new Person(parrot); // 생성자 주입 방식
        p.setName("Ella");
